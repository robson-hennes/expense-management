...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.incomes.update", [$income->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="users_id">{{ trans('cruds.income.fields.users') }}</label>
                <select class="form-control select2 {{ $errors->has('users') ? 'is-invalid' : '' }}" name="users_id" id="users_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('users_id') ? old('users_id') : $income->users->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.income.fields.users_helper') }}</span>
            </div>
            <div class="form-group">
